// Parses AI Sundays (formerly AI Workshop) markdown notes + planning JSON into src/data.json
...
// People data (names, aliases, LinkedIn, photos, who came to which session) is
// written HERE, a server-only module behind the signed-in /api/members route,
// and never into src/data.json, which is a static import and ships in the public
// JS bundle. The bundle keeps counts only.
const MEMBERS_OUT = join(ROOT, 'api', '_members-data.js');
// The set of dates that are real sessions. /api/photos PATCH validates a move
// target against this: without it any signed-in member could move photos to an
// invented date, where no view would ever show them again — a delete in all but
// name, on a route that is deliberately NOT the organizer-only delete.
const SESSIONS_OUT = join(ROOT, 'api', '_sessions-data.js');
const PROFILES_FILE = join(ROOT, 'data', 'members-profile.json');
...
if (process.env.VERCEL && existsSync(OUT_FILE) && existsSync(MEMBERS_OUT)) {
...
if (!members.length && existsSync(MEMBERS_OUT)) {
  members = (await import(MEMBERS_OUT)).default?.members || [];
}
...
// Public bundle: sessions without their attendee lists (count only), and a member
// count instead of the directory.
const publicSessions = sessions.map(({ attendees, ...s }) => ({ ...s, attendeeCount: (attendees || []).length }));
const out = {
  generatedAt,
...
// Server-only: the directory, the profile map and who came when.
const profiles = readJson(PROFILES_FILE) || {};
const attendeesByDate = Object.fromEntries(sessions.map((s) => [s.date, s.attendees || []]));
const membersModule = `// GENERATED by scripts/build-data.js. Do not edit by hand.
// Served only through the signed-in /api/members route. Never import from src/.
export default ${JSON.stringify({ generatedAt, members, profiles, attendeesByDate }, null, 2)};
`;

// Every date the app considers a real session: the ones with notes, plus the
// scheduled ones that have not happened yet (a photo can legitimately be filed
// against an upcoming session). Sorted and de-duplicated so the module is stable
// across rebuilds and does not churn the diff.
const sessionDates = [...new Set([
  ...sessions.map((s) => s.date),
  ...(schedule.upcoming || []).map((s) => s.date),
])].filter((d) => /^\d{4}-\d{2}-\d{2}$/.test(d)).sort();
const sessionsModule = `// GENERATED by scripts/build-data.js. Do not edit by hand.
// The allowed targets for a photo move (/api/photos PATCH). Never import from src/.
export const SESSION_DATES = ${JSON.stringify(sessionDates, null, 2)};
`;
...
if (membersModule.includes('<!--')) {
  console.error('build-data: an HTML comment leaked into the members output.');
  process.exit(1);
}

writeFileSync(OUT_FILE, serialised);
writeFileSync(MEMBERS_OUT, membersModule);
writeFileSync(SESSIONS_OUT, sessionsModule);
console.log(`build-data: ${sessions.length} sessions, ${members.length} members, ${schedule.upcoming.length} upcoming, ${allActions.length} open actions, ${sessionDates.length} session dates → src/data.json + api/_members-data.js + api/_sessions-data.js`);
