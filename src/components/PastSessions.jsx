  ...
  return (
    <div className="card card-pad h-full">
      <div className="flex items-baseline justify-between">
        ...
        <div className="text-xs text-muted num">{sessions.length} logged</div>
      </div>
...
function SessionRow({ s }) {
  const headline =
    s.demos.length > 0
      ...
      : (s.summary?.split('\n')[0] || '—');
  return (
    <li className="flex gap-3 text-sm border-l-2 border-border pl-3 py-1">
      <span className="num text-muted w-10 flex-shrink-0">#{s.number}</span>
      <span className="num text-muted w-24 flex-shrink-0 opacity-70">{fmtDate(s.date)}</span>
      <span className="flex-1 text-foreground truncate" title={headline}>{headline}</span>
      <span className="text-[11px] text-muted num flex-shrink-0">{s.attendees.length} ppl</span>
    </li>
  );
}
...
function GapRow({ g }) {
  return (
    <li className="flex gap-3 text-sm border-l-2 border-warn/40 pl-3 py-1 bg-warn/5 rounded-r-md">
      ...
      <span className="num text-muted w-24 flex-shrink-0">{fmtDate(g.from)} →</span>
      <span className="flex-1 text-muted italic">{g.reason}</span>
    </li>
  );
}
