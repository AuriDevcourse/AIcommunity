...
- Home is now the bare URL (no `#home`); other tabs stay `#tab`. Cleans the post-login URL too. (App.jsx hash-sync effect. UNCOMMITTED.)
- SIGN-IN FIX (Supabase dashboard, not code): Google OAuth was bouncing to the wrong domain because Supabase's Site URL + Redirect URLs pointed at the vercel URL, not the real domain. Real domain is **aisundays.org** (apex + www both serve the app via Vercel). Fix in Supabase project `iogwikfvzxfblwuuvtmq` → Authentication → URL Configuration: Site URL = `https://aisundays.org`; Redirect URLs add `https://aisundays.org`, `https://aisundays.org/**`, `https://www.aisundays.org`, `https://www.aisundays.org/**` (+ localhost). Config is instant, no redeploy. Also recommend a www→apex redirect in Vercel Domains so there is one canonical origin (sessions are per-origin). Auth uses PKCE (`?code=`), flowType pkce in supabase.js, so the hash router is safe.
- OPEN idea (not started): switch other tabs from `#learn` to path routing `/learn` — needs Vercel SPA rewrites + a routing refactor + retest of recaps/poll deep-links/sign-in. Medium risk; do after sign-in is confirmed working.
- Add-a-project on the Projects tab (UNCOMMITTED, not yet pushed): signed-in members add showcase cards. New `api/showcase.js` + `api/_showcase.js` (Upstash in prod / `data/showcase-store.json` in dev, mirrors `_projects.js`; public GET, member POST add/delete, rate-limited, http-only links, field caps). `Projects.jsx` fetches `/api/showcase`, merges with the seed `projects.json`, and opens an Add modal (picture upload via `/api/upload-image` + `compressImage`, Live/Beta/In-progress label, headline, subtitle, short description, link, 3 tags, built-by). Delete-own on your cards. Verified: GET returns list, POST 401 without auth, tab renders, build green. NEEDS Upstash in prod to persist (same KV the member board uses). Dev gotcha: new `/api/*` routes must be registered in `vite.config.js` (done for showcase) and the dev server restarted.
